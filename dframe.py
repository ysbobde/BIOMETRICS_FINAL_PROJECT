...
def reset_voter_list():
    df = pd.DataFrame(columns=['voter_id','Aadhar','Name','Gender','City','Passw','hasVoted'])
    df=df[['voter_id','Aadhar','Name','Gender','City','Passw','hasVoted']]
    df.to_csv(path/'voterList.csv')
...
def taking_data_voter(Aad_num,name,gender,city,passw):
    df=pd.read_csv(path/'voterList.csv')
    df=df[['voter_id','Aadhar','Name','Gender','City','Passw','hasVoted']]
    row,col=df.shape
    if row==0:
